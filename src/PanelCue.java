...
public class PanelCue {
    ...
    private static final ActionListener actions = e -> {
        ...
        if (Objects.equals(btnCueSource.getText(), "+ Cue")) {
            // Enable Buttons & Menu when adding first cue
    ...
    private static void ChangeDelete() {
        if (boolDelete) {
            ...
            boolean enable = intAmountOfLabels != 0;
    ...
    private static void delCue(int button) {
        ...
        intAmountOfLabels--;
        // If there are no more Cues, then disabled DelButton & Menu
        if (intAmountOfLabels == 0) {
            buttonDelete.setEnabled(false);
            MenuBar.getSave().setEnabled(false);
        }
        WindowMixer.getPanelArray()[0].repaint();
    }
